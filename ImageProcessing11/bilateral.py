...
def my_bilateral(src, msize, sigma, sigma_r, pad_type='zero'):
    ...
    ####################################################################################################
    (h, w) = src.shape
    src_pad = my_padding(src, (msize // 2, msize // 2), pad_type)
    dst = np.zeros((h, w))
    dk, dl = np.mgrid[-(msize // 2):(msize // 2) + 1, -(msize // 2):(msize // 2) + 1]
    for i in range(h):
        print('\r%d / %d ...' %(i,h), end="")
        for j in range(w):
            mask = np.exp(-(dk ** 2) / (2 * sigma ** 2) - (dl ** 2) / (2 * sigma ** 2)) * np.exp(-((src[i, j] - src_pad[i:i+msize, j:j+msize]) ** 2) / (2 * sigma_r ** 2))
            mask /= np.sum(mask)
            val = np.sum(src_pad[i:i + msize, j:j + msize] * mask)
            dst[i, j] = val
    return dst
